...
Cooling Deserts EDA
===================
Thesis: Cooling Deserts for Renters — Heat Equity Inside the NYC Housing System
Author: Tahreem Ashraf
Date: 2026-03-25

Phases:
  1. Structural audit of each dataset
  2. Data quality assessment (missing, bounds, duplicates)
  3. Univariate analysis (descriptive stats, normality, outliers)
  4. Bivariate analysis (Spearman/Pearson correlations, Kruskal-Wallis)
  5. Multivariate patterns (correlation matrix, VIF)
  6. Spatial distribution (walksheds, NYCHA vs HVI zones)
  7. Borough-level comparison (Kruskal-Wallis + Dunn post-hoc)
  8. Findings report

Run from project root with the .venv active:
  python analysis/02_exploratory/cooling_deserts_eda.py
"""
...
section("PHASE 8 — FINDINGS REPORT")

findings = """
┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 1: Race is the strongest spatial predictor of heat vulnerability  │
│ VARIABLES: pct_black × hvi_score                                          │
│ EVIDENCE: Spearman r=+0.601, p<0.001 (n=2130 tracts)                     │
│ Stronger than income (r=-0.479) or poverty (r=+0.425)                    │
│ EQUITY IMPLICATION: Race operates as an independent axis of heat risk,   │
│ not merely a proxy for income — replicating Shaker et al. at tract level  │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 2: The Bronx is the city's primary cooling desert                │
│ VARIABLES: hvi_score, pct_rent_burden_50_plus, borough                   │
│ EVIDENCE: 61.6% of Bronx tracts qualify as cooling deserts               │
│ (HVI≥4 + severe rent burden >30%). Bronx median HVI=4.28 vs              │
│ Manhattan 2.35. KW H=525.9, p<0.001, η²=0.246 across boroughs           │
│ EQUITY IMPLICATION: Bronx renters face simultaneous heat exposure        │
│ and structural inability to self-protect                                  │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 3: AC ownership at NTA level masks severe tract-level inequality │
│ VARIABLES: pct_households_ac (HVI), hvi_rank                             │
│ EVIDENCE: NTA-level AC ranges only 75.8–98.4%. KW H=100.1, p<0.001,    │
│ η²=0.525 — AC drops from 94.9% (rank 1) to 83.6% (rank 5)              │
│ EQUITY IMPLICATION: NTA averages compress the dispersion seen at tract   │
│ level — the HVI underestimates cooling access gaps                        │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 4: 70.6% of NYC tracts have no Cool It! site within 0.5 miles   │
│ VARIABLES: cool_it_sites coordinates, tract centroids                    │
│ EVIDENCE: Spatial walkshed analysis. Brooklyn: 28.4% coverage,          │
│ Queens: 15.4%, Staten Island: 0%                                         │
│ EQUITY IMPLICATION: Proximity-based access is inadequate — most          │
│ vulnerable tracts lack walkable outdoor cooling infrastructure            │
└───────────────────────────────────────────────────────────────��──────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 5: 62.3% of NYCHA developments are in HVI zones 4–5             │
│ VARIABLES: nycha_geo, hvi_zcta                                            │
│ EVIDENCE: 132/212 NYCHA developments with HVI scores in zones 4–5.      │
│ Bronx NYCHA: 93% in zones 4–5, mean HVI=4.50                            │
│ EQUITY IMPLICATION: Public housing residents are disproportionately      │
│ concentrated in the highest heat risk zones                               │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 6: Building age does not predict heat vulnerability or burden    │
│ VARIABLES: pct_housing_pre_1980 × hvi_score, × pct_rent_burden_50_plus  │
│ EVIDENCE: Both correlations non-significant (r=-0.026 ns, r=-0.019 ns)  │
│ EQUITY IMPLICATION: Pre-1980 housing is near-universal in NYC (median   │
│ 80–88% by borough), so it cannot serve as a differentiating index        │
│ variable. Energy infrastructure capacity data would be needed instead    │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 7: Hispanic-majority tracts carry most of the LEP heat risk     │
│ VARIABLES: pct_hispanic × pct_limited_english, pct_black × pct_lep      │
│ EVIDENCE: Hispanic × LEP: r=+0.502; Black × LEP: r=-0.246              │
│ EQUITY IMPLICATION: Language-access interventions should be targeted at  │
│ Hispanic-majority cooling desert tracts (214 tracts meet triple criteria) │
└──────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────┐
│ FINDING 8: NYCHA rent is affordable — the hidden constraint is energy    │
│ VARIABLES: avg_monthly_rent_usd, tract median_household_income           │
│ EVIDENCE: NYCHA median rent = 0.21x–0.46x of 30%-of-income threshold   │
│ across boroughs. Rent itself is not the barrier.                         │
│ EQUITY IMPLICATION: AC surcharge and electricity costs (missing from     │
│ current data) are the likely access constraint — confirms the            │
│ energy insecurity literature. DOE LEAD data is the priority acquisition  │
└──────────────────────────────────────────────────────────────────────────┘
"""
print(findings)
...
quality = """
  ...
  ─────────────────────────────────────────────────────────────────────────
  Medium    hvi_nta_components      6 duplicate SI NTA codes          Drop duplicates (keep first)
  High      hvi_nta_components      poverty_level / surface_temp_level Use numeric columns instead
                                    35.9% coded as 'unknown'
  Low       nyc_tract_indicators    3 disability columns (x, y, final) Use pct_disability only
  Low       nyc_tract_indicators    32 tracts missing income (1.4%)   Acceptable; note as limitation
  High      nycha_development_book  senior_development 84% missing    Treat NaN as non-senior
  High      nycha_development_book  electricity column 74% missing    Drop column
  Medium    nycha_development_book  Column typo 'community_distirct'  Rename in all scripts
  High      nycha GeoJSON vs book   128-record gap                    Join via tds_num, not row order
  High      cool_it_sites           0 sites in Staten Island          Flag as coverage gap; note
                                                                      in all proximity analyses
  High      heat forecast ↔ HVI     Low match rate on nta_code_4      Aggregate heat to 4-char NTA
                                    (30/189 direct matches)           before joining to HVI
"""
print(quality)

section("WHAT CURRENT DATA CANNOT ANSWER")
gaps = """
  GAP                                    MISSING DATASET
  ──────────────────────────────────────────────────────────────────
  Actual AC usage vs ownership           NYCHVS energy cost variables
  Energy costs as % of income            DOE LEAD energy burden data
  Indoor cooling center hours/ADA status Emergency cooling centers dataset
  Building-level heat/hot water failures HPD Heat & Hot Water Violations
  NYCHA AC hookup infrastructure         Internal NYCHA infrastructure data
"""
print(gaps)

section("PRIORITY FOLLOW-UP ANALYSES (after NYCHVS + cooling centers acquired)")
followups = """
  1. AC usage gap model: use NYCHVS UTILCOSTS_SUMMER × RENTBURDEN_CAT to
     estimate actual cooling access (vs ownership) by borough. Compute
     'effective cooling access' = AC ownership × (1 - non-use rate due to cost)
     and overlay against HVI rank to show the true vulnerability undercount.

  2. Energy burden × heat exposure regression: merge DOE LEAD tract-level
     energy burden data with HVI score and rent burden. Test whether energy
     costs explain variance in heat outcomes above and beyond income and
     AC ownership — the missing causal link in the current index.

  3. Full accessibility analysis for indoor cooling centers: once coordinates
     and hours data are available, compute realistic access buffers (transit
     time, not distance) weighted by workforce hours and disability status.
     Compare to Cool It! outdoor site coverage to map the full cooling
     infrastructure gap for each borough.
"""
print(followups)

print("\n✓ EDA complete. All results above are reproducible from this script.")
print("  Output feeds into: analysis/03_statistical/ for index construction.")
